...
```shell
$ cd ~/catkin_ws/src
$ git clone https://github.com/KMiyawaki/oit_minibot_light_v5.git
$ cd ~/catkin_ws/src/oit_minibot_light_v5
$ ./install.sh
# 自動的に再起動される。
```
...
```shell
$ roslaunch oit_minibot_light_v5 devices.launch use_camera:=false
...
```shell
$ roslaunch oit_minibot_light_v5 teleop.launch use_camera:=false
...
```shell
$ roslaunch oit_minibot_light_v5 mapping.launch use_camera:=false
# teleop:=joy をつけるとジョイスティックによるテレオペが起動する。
# カメラが使える場合は use_camera:=false 無しに起動すれば良い。
```
...
```shell
$ roscd oit_minibot_light_v5/maps
...
```shell
$ roslaunch oit_minibot_light_v5 navigation.launch map_name:=test use_camera:=false
# カメラが使える場合は use_camera:=false 無しに起動すれば良い。
...
$ roslaunch oit_minibot_light_v5 stage_navigation.launch map_name:=test
```

### Stage によるナビゲーション

```shell
$ roslaunch oit_minibot_light_v5 stage_navigation.launch map_name:=test # map_name:=以降の文字をナビゲーション時に利用する地図名に変更する。
```
...
以下のコマンドでロボットのソフトをアップデートできる。ただし、パッケージ`oit_minibot_light_v5`に加えた変更は全て消去される。

```shell
$ roscd oit_minibot_light_v5
$ git fetch
$ git checkout main
